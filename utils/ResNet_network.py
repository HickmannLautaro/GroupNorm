...
class resnet_block_type_a(tf.keras.Model):
    ...
    def call(self, input_tensor, training=False, **kwargs):
        ...
        if self.stride == 2:
            x = x + self.conv_downsample(input_tensor)
        else:
            x = x + input_tensor
...
class res_net(tf.keras.Model):
   ...
   """
    ...
    def __init__(self, n, normalization, classification_classes, **kwargs):
...
def get_resnet_n(arguments):
    ...
    norm = arguments['norm']
    model = res_net(n=arguments['ResNet'], normalization=norm, classification_classes=10)
    ...
    res_block.build(input_shape=(None, 32, 32, 16))
    res_block.summary()
